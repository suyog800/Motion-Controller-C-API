...
/* USER CODE BEGIN PFP */
static void app_process_can_rx(void);
/* USER CODE END PFP */
...
//  4 	Torque (Iq) Control Mode
  ...
  if (motion_drive_set_mode_of_operation(&drive1, 1U) != MOTION_DRIVE_OK)
  {
	  Error_Handler();
  }
  if (motion_drive_set_iq_gains(&drive1, 0.4f, 116.0f) != MOTION_DRIVE_OK)
	{
	  Error_Handler();
	}
  if (motion_drive_set_velocity_gains(&drive1, 40U, 5218U, 0U, 0U) != MOTION_DRIVE_OK)
    {
  	  Error_Handler();
    }
  if (motion_drive_set_position_gains(&drive1, 1400U, 0U, 0U, 0U) != MOTION_DRIVE_OK)
    {
  	  Error_Handler();
    }
  if (motion_drive_set_accel_decel(&drive1, 500U, 500U) != MOTION_DRIVE_OK)
    {
  	  Error_Handler();
    }
  if (motion_drive_set_controlword(&drive1,15U) != MOTION_DRIVE_OK)
    {
  	  Error_Handler();
    }
...
//  motion_drive_set_target_iq(&drive1,0.5);
//
//  motion_drive_set_velocity(&drive1, 1000);
  motion_drive_set_target_position_and_velocity(&drive1, 50.0, 1000.0);
//  HAL_Delay(10000);
//  motion_drive_set_velocity(&drive1, 0);
//
//  motion_drive_set_target_iq(&drive1,0.0);
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, Error_LED_Pin|HOLD_Pin|WP_Pin|SPI3_CS2_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : Error_LED_Pin HOLD_Pin WP_Pin SPI3_CS2_Pin */
  GPIO_InitStruct.Pin = Error_LED_Pin|HOLD_Pin|WP_Pin|SPI3_CS2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
static void app_process_can_rx(void)
{
    ...
    do
    {
        ...
        {
            /*
             * Update NMT state from heartbeat.
             */
            (void)motion_drive_process_heartbeat(&drive1, &rx_frame);

            /*
             * Update measured feedback values from TPDO1..TPDO4.
             */
